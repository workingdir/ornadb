...
fn migration_registry_is_a_strict_contiguous_sequence() {
    ...
    assert_eq!(MIGRATIONS[53].name, "sealed invocation catalogue witnesses");
    assert_eq!(
        MIGRATIONS[54].name,
        "sealed invocation orphan replacement receipt"
    );
    ...
    assert!(MIGRATIONS[49].data_step.is_none());
    assert!(MIGRATIONS[50].data_step.is_none());
}
