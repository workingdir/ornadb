...
fn is_finite_list_integer_aggregate_pipeline_stage(expression: &Expr, scope: &Scope) -> bool {
    ...
    matches!(operation, Some("sum" | "min" | "max"))
}
...
fn infer_finite_list_integer_aggregate(
    ...
    if rows != Type::List(Box::new(Type::Int)) {
        if matches!(&rows, Type::List(element) if is_absolute_affine_temperature(element)) {
            diagnostics.push(diag(
                DIAG_TYPE,
                format!("cannot {operation} absolute affine quantities"),
            ));
        } else {
            diagnostics.push(diag(
                DIAG_TYPE,
...
fn infer_finite_list_one(
    ...
    if let Some(index) = slots[1] {
        let callback = infer_finite_list_predicate(
            "one",
            &arguments[index].value,
...
fn infer_finite_list_predicate(
    operation: &str,
    expression: &Expr,
    parameter: Type,
    scope: &Scope,
    ...
    } = expression
    {
        let [lambda_parameter] = parameters.as_slice() else {
            diagnostics.push(diag(
                DIAG_TYPE,
                format!("finite-list {operation} predicate must take one parameter"),
            ));
            return Inferred {
                ty: Type::Error,
                effects: EffectSummary::default(),
        ...
        let Pattern::Name(name, _) = &lambda_parameter.pattern else {
            diagnostics.push(diag(
                DIAG_UNSUPPORTED,
                format!("finite-list {operation} predicate pattern is outside this semantic slice"),
            ));
            return Inferred {
                ty: Type::Error,
                effects: EffectSummary::default(),
    ...
    let Type::Function {
        ...
        diagnostics.push(diag(
            DIAG_TYPE,
            format!("finite-list {operation} predicate must be a one-parameter function"),
        ));
        return Inferred {
            ty: Type::Error,
...
/// Checks the finite-list `every(rows, predicate)` and
/// `exists(rows, predicate)` signatures. Both operations always return Bool;
/// their empty-list identities and short-circuit evaluation belong to the
/// runtime, while callback effects are retained in the semantic summary.
fn infer_finite_list_every_exists(
    operation: &str,
    arguments: &[orna_syntax_v1::Argument],
    input: Option<Inferred>,
    scope: &Scope,
    ...
    effects.join(&callback.effects);
    Inferred {
        ty: if !malformed && callback.ty == Type::Bool {
            Type::Bool
        } else {
            Type::Error
        },
...
/// remain outside the integer-only finite-list aggregate slice. This is
/// deliberately limited to the core Celsius identity: user-defined unit
/// declaration resolution remains outside this read-only slice, and
/// unrecognised collection pipelines retain their prior conservative
/// diagnostics.
