...
use orna_core::{
    ...
    revision::{ActiveDatabaseRevision, DeployableRevision, RevisionPair, StoredSourceRevision, StoredSourceUnit},
};
...
impl SqliteRevisionStore {
    ...
    async fn seed_pair(&self) -> Result<BootstrapRevision, SqliteError> {
        ...
        self.connection.execute(
            ...
            [
                ...
                Value::Blob(bundle.to_bytes().to_vec()), Value::Blob(source_bundle_hash.to_bytes().to_vec()),
    ...
    async fn load_active(&self) -> Result<ActiveDatabaseRevision, SqliteError> {
        ...
        let Some(row) = rows.next().await? else { return Err(SqliteError::InvalidPersistedData("database has not been bootstrapped")) };
        let source_id = SourceRevisionId::from_bytes(id16(row.get::<Vec<u8>>(0)?, "source revision id")?);
        let catalogue_id = CatalogueRevisionId::from_bytes(id16(row.get::<Vec<u8>>(1)?, "catalogue revision id")?);
        ...
        while let Some(unit) = unit_rows.next().await? {
            units.push(StoredSourceUnit::new(
                ...
                u32::try_from(unit.get::<i64>(1)?).map_err(|_| SqliteError::InvalidPersistedData("source unit ordinal must fit u32"))?,
                unit.get::<String>(2)?, unit.get::<String>(3)?,
                digest32(unit.get::<Vec<u8>>(4)?, "source unit hash")?,
            ).map_err(|e| SqliteError::Domain(e.to_string()))?);
        }
        if source_bundle_digest(&units).map_err(|e| SqliteError::Domain(e.to_string()))? != bundle_hash {
            return Err(SqliteError::InvalidPersistedData("source bundle hash mismatch"));
        }
        let source = StoredSourceRevision::new(bundle, source_id, None, units, bundle_hash, source_hash)
            .map_err(|e| SqliteError::Domain(e.to_string()))?;
        ...
        while let Some(schema) = schema_rows.next().await? {
            let parts = schema.get::<String>(1)?.split('\u{1f}').map(str::to_owned).collect::<Vec<_>>();
            schemas.push(SchemaDefinition::new(
                SchemaId::from_bytes(id16(schema.get::<Vec<u8>>(0)?, "schema id")?),
                QualifiedSemanticName::new(parts).map_err(|e| SqliteError::Domain(e.to_string()))?,
            ));
        }
        let catalogue = CatalogueSnapshot::new(catalogue_id, schemas, Vec::new())
            .map_err(|e| SqliteError::Domain(e.to_string()))?;
        ActiveDatabaseRevision::new(RevisionPair::new(source_id, catalogue_id), source, catalogue,
            catalogue_hash, Vec::new(), Vec::new(), Vec::new(), Vec::new())
            .map_err(|e| SqliteError::Domain(e.to_string()))
    }
    ...
    pub async fn apply(&self, candidate: &DeployableRevision) -> Result<ActiveDatabaseRevision, StorageError<SqliteError>> {
        if !candidate.source().units().is_empty()
            ...
            || !candidate.origins().iter().all(|origin| matches!(origin.identity(), orna_core::revision::DefinitionIdentity::Schema(_)))
            || !candidate.expressions().is_empty()
            || !candidate.new_function_revisions().is_empty()
            || !candidate.references().is_empty()
            || candidate.catalogue_hash_context().version() != orna_core::revision::CatalogueHashVersion::Version1
        { return Err(StorageError::Backend(SqliteError::UnsupportedApply)); }
        let active = self.load_pair().await.map_err(StorageError::Backend)?.ok_or(SqliteError::InvalidPersistedData("database has not been bootstrapped")).map_err(StorageError::Backend)?;
        ...
        let tx = turso::transaction::Transaction::new_unchecked(&self.connection, turso::transaction::TransactionBehavior::Immediate)
        ...
        for origin in candidate.origins() {
            let schema_id = match origin.identity() { orna_core::revision::DefinitionIdentity::Schema(id) => id, _ => unreachable!() };
            let schema = candidate.candidate().schema_by_id(schema_id).ok_or(SqliteError::InvalidPersistedData("schema origin has no candidate schema")).map_err(StorageError::Backend)?;
...
fn id16(value: Vec<u8>, field: &'static str) -> Result<[u8; 16], SqliteError> { value.try_into().map_err(|_| SqliteError::InvalidPersistedData(field)) }
fn digest32(value: Vec<u8>, field: &'static str) -> Result<orna_core::revision::Sha256Digest, SqliteError> { Ok(orna_core::revision::Sha256Digest::from_bytes(value.try_into().map_err(|_| SqliteError::InvalidPersistedData(field))?)) }
