...
pub fn check_standard_source_v11(
    ...
    let math_unit_index = source_units
        .iter()
        .position(|unit| unit.logical_path() == "std/math.orna")
        .ok_or(StandardLibraryCheckError::SourceMismatch)?;
    if math_unit_index != source_units.len() - 1 {
        return Err(StandardLibraryCheckError::SourceMismatch);
    }
    let math_unit = &source_units[math_unit_index];
    let math_unit_id = math_unit.id();
    ...
    let math_schema = snapshot
        ...
        .schemas()
        .iter()
        .find(|schema| schema.name().parts() == ["std", "math"])
        .ok_or(StandardLibraryCheckError::SourceMismatch)?;
    ...
    let parent_origins = snapshot
        .origins()
        .iter()
        .filter(|origin| origin.source().source_unit() != math_unit_id)
        .cloned()
        .collect::<Vec<_>>();
    ...
    let math_bundle = SourceBundle::new([SourceUnit::new(
