...
fn metadata(profile: &CompactOvbProfile, descriptors: Vec<OvbRaw>) -> Vec<KeyValue> {
    ...
    vec![
        ...
        KeyValue::new("orna.encoder".into(), Some("uuid-proof-v1".into())),
    ]
}
...
fn parquet(
    ...
    for id in physical_ids {
        let repetition = if optional { "OPTIONAL" } else { "REQUIRED" };
        let annotation = annotation.map_or_else(String::new, |value| format!(" ({value})"));
        message.push_str(&format!(
            " {repetition} FIXED_LEN_BYTE_ARRAY({width}) f_{}{annotation};",
            Uuid::from_bytes(*id).simple()
        ));
    }
    message.push('}');
    let schema = Arc::new(parse_message_type(&message).unwrap());
    let columns = physical_ids.iter().copied().map(descriptor).collect();
    let properties = Arc::new(
        WriterProperties::builder()
            ...
            .set_key_value_metadata(Some(metadata(profile, columns)))
            .build(),
    );
    ...
    for column_values in values {
        ...
        let values = column_values
            .chunks(width)
            .map(|value| FixedLenByteArray::from(value.to_vec()))
            .collect::<Vec<_>>();
        if optional {
            let definitions = [0_i16, 1_i16];
...
fn with_page_checksums(bytes: Vec<u8>) -> Vec<u8> {
    ...
    for (row_group, group) in row_groups.iter_mut().enumerate() {
        ...
        for (column, meta) in columns.iter_mut().enumerate() {
            ...
            let data_offset = *data_offset as i64;
            *meta = meta
                .clone()
                .into_builder()
                .set_data_page_offset(data_offset)
                .set_dictionary_page_offset(meta.dictionary_page_offset().map(|_| start))
...
fn expected_uuid(value: [u8; 16]) -> Vec<u8> {
    CanonicalValue::new(uuid_raw(value)).unwrap().encode().unwrap()
}

fn expected_tuple(values: &[[u8; 16]]) -> Vec<u8> {
    CanonicalValue::new(OvbRaw::Tag(
        60015,
        Box::new(OvbRaw::Array(values.iter().copied().map(uuid_raw).collect())),
    ))
    .unwrap()
    .encode()
    .unwrap()
...
#[test]
fn scalar_uuid_decodes_to_exact_tag_37_network_order_ovb() {
    ...
    assert_eq!(
        ...
        vec![expected_uuid(KEY_A)]
    );
}

#[test]
fn composite_uuid_decodes_in_declared_primary_key_order() {
    ...
    assert_eq!(
        CompactParquetKeySource::decode_verified_bytes(&profile, TABLE, &bytes, 1).unwrap(),
        vec![expected_tuple(&[KEY_B, KEY_A])]
    );
}

#[test]
fn malformed_uuid_width_annotation_null_encoding_and_row_count_fail_closed() {
    let profile = profile(&[KEY_A]);
    let wrong_width = parquet(
        &profile,
        &[KEY_A],
        &[KEY_A[..15].to_vec()],
        15,
    ...
    assert!(matches!(
        CompactParquetKeySource::decode_verified_bytes(&profile, TABLE, &wrong_width, 1),
        Err(CompactParquetError::UnsupportedKeyMapping)
    ));

    let missing_annotation = parquet(
        &profile,
        &[KEY_A],
        &[KEY_A.to_vec()],
        16,
    ...
    assert!(CompactParquetKeySource::decode_verified_bytes(
        &profile,
        TABLE,
        &missing_annotation,
        1
    ...
    assert!(CompactParquetKeySource::decode_verified_bytes(&profile, TABLE, &nullable, 2).is_err());

    let unsupported = parquet(
        &profile,
        &[KEY_A],
        &[KEY_A.to_vec()],
        16,
    ...
    assert!(matches!(
        CompactParquetKeySource::decode_verified_bytes(&profile, TABLE, &unsupported, 1),
    ...
    assert!(matches!(
        CompactParquetKeySource::decode_verified_bytes(&profile, TABLE, &valid, 2),
