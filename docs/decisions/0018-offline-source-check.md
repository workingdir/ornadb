...
to active construction, including raw version-1 recovery.
`ActiveOrRecoveredStandard` identifies the standard catalogue supplied to a
standard snapshot or active context. Current version-1 recovery has no standard
context. The raw standard sentinel proof is deferred to and required by work
ADR 0016's later `feat(postgres): decode standard revisions` row. The three
`Deployable*` roles identify the three fields of `DeployableRevisionInput`
without combining their evidence.

The sentinel can never identify an active, stored, prepared, or recovered
catalogue revision. Active-revision construction fails closed if an application
or standard catalogue uses it. Current version-1 recovery rejects the sentinel
only in its raw application catalogue. Work ADR 0016's later standard-revision
decoder rejects it in the raw standard catalogue. `DeployableRevision`
construction checks its expected-base catalogue, explicit parent catalogue, and
candidate catalogue in that order and rejects the sentinel at every position
with the corresponding typed role before another deployable invariant or
allocation.
...
| --- | --- | --- |
| Command shape | source-check leaf plus one path; optional leading `--color auto|always|never`; missing command or path; empty path; other flag placement; extra path; `-`; `--`; literal wildcard; shell expansion to multiple paths | The three-token leaf and its optional leading colour flag continue. Every other invalid shape prints the exact four-line usage and exits `2`. A literal wildcard is an ordinary path and uses the read result for that literal name. Orna performs no glob expansion and never reads standard input. |
...
recovery integration test owns only the raw recovered version-1 application
sentinel position. Work ADR 0016's later standard-revision decoder owns the raw
standard sentinel position.
...
| `feat(compiler): check verified new applications` | `crates/orna-core/src/revision.rs`; `crates/orna-compiler/src/lib.rs`; `crates/orna-compiler/src/resolver.rs` | Implement and export exact `NewApplicationCheckError` and `check_new_application`; accept only `&CheckedStandardLibrary`, return `StandardApplicationCheckReport`, reject zero and multiple source units before `CatalogueSnapshot::new`, map empty-sentinel empty-vector catalogue construction to `Catalogue { source }`, then map context construction to `Context { source }`, with no `panic!` or `expect`; reserve and expose exact `EMPTY_APPLICATION_CATALOGUE_REVISION_ID`; add exact `DurableCatalogueRevisionRole` and `ReservedOfflineCheckCatalogueRevision`; reject the sentinel from direct active application and standard construction, raw recovered application construction, and deployable expected-base, parent, and candidate positions; label the successful distinct checked bundle with the sentinel; and use the same production-private construction seam as the public path to supply typed `CatalogueSnapshotError` plus hostile context, proving exact `Catalogue` precedence, display, and source without a public seam or test-only production branch, while legacy preparation cannot accept the distinct report or bundle at compile time and durable core rejection remains. The later work ADR 0016 standard-revision decoder owns raw standard recovery. This row starts only after all work ADR 0016 prerequisites above. |
| `test(postgres): reject offline application catalogue identity` | `crates/orna-kernel-postgres/tests/recovery.rs` | Insert the all-zero identity only as the raw recovered version-1 application catalogue. Prove exact `RevisionInvariantError::ReservedOfflineCheckCatalogueRevision { revision, role: ActiveOrRecoveredApplication }`, its exact display and absent error source, no returned active revision, no repair, and no write with a complete table snapshot before and after recovery. Current version-1 recovery has no standard context. Work ADR 0016's later `feat(postgres): decode standard revisions` row owns and requires the raw standard-catalogue sentinel proof with `ActiveOrRecoveredStandard`. |
