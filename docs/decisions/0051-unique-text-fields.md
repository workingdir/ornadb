...
| `feat(compiler): admit unique Text fields` | `crates/orna-compiler/src/resolver.rs`; `crates/orna-compiler/src/prepare.rs` | Add the DeepSeek-owned acceptance and rejection tracers, then admit only the exact Text and retained required-Reference source shapes. |
