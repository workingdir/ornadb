...
| `test(postgres): prove the standard lifecycle` | `crates/orna-kernel-postgres/tests/apply.rs`, `crates/orna-kernel-postgres/tests/recovery.rs`, `justfile` | The one installed-standard authority drives fresh install, v1 upgrade, replay, restart, tamper rejection, and exact physical storage on PostgreSQL 18, extending the atomic-apply row's same-context value-storage round trip without a second standard authority. |
