...
| Resolved value identity migration | Direct `ResolvedType::Value(TypeId)` construction; tag `4`; the exact three new source-free canonical and revision errors, fields, displays, sources, and slot order; every function parameter, including hostile CLIENT parameter, coverage; exhaustive recovery fixture/helper matches converted before the variant; transitional version-2 scalar acceptance, strict constructor-valid in-memory preflight rejection, legacy `Value` rejection before atomic apply, and final public-hash and construction rejection; version-1 byte, hash, row, and error preservation; all migration-0008 DDL declarations, named replacements, length checks, composite keys, deferrability, and unchanged object keys; atomic installation then normal same-context apply through the one installed-standard authority; non-golden changed `TypeId`; wrong, missing, and crossed pin; restart; physical capability, unsupported, transient, and `VOID` contracts; no-DDL physical parity; exact PostgreSQL storage type | The migration has no sidecar and no `Named` reuse. The two public canonical hash entry points and revision construction scan every field, every function parameter, `ROWS`, and `SINGLE` slot in the stated order. Version 1 rejects `Value`; final version 2 rejects `Scalar`; version 2 rejects a missing pinned value definition. A hostile CLIENT parameter proves the same three rules and proves that transition-only recovery acceptance cannot bypass strict persistence validation. Recovery fixtures preserve raw version-1 and transitional version-2 scalar facts through the non-authoritative accessors without a wildcard. The production-private preflight proves `validate_persistable_catalogue` rejects every new version-2 scalar candidate after expected-base and context checks, before materialisation, planning, or SQL writes, through `PostgresKernelError::CandidateRevisionInvariant`; until atomic apply, the legacy encoder rejects `Value`. Atomic standard installation establishes the sole version-2 context and then normal same-context apply persists the exact catalogue context, function semantic versions, value tuples, and value-reference target pin rather than version-1 defaults. SQL accepts a value tuple only with the matching catalogue pin and standard value row. Recovery retains the stated tuple, pin, definition, canonical, then physical error order. A changed non-golden `TypeId` round-trips through field, parameter, `ROWS`, and `SINGLE` return storage and restart. One core `PhysicalCatalogue` authority exposes only ordered `CreateObject` and `CreateField` accessors to PostgreSQL and makes equal allowed version-1 scalar and version-2 value contracts produce the same physical PostgreSQL type without DDL. The named value errors have the stated field, display, source, and gate order. |
...
* the direct resolved-value migration has no scalar sidecar and no `Named`
  ...
  version-2 value rejection. Before decoder migration, constructor-valid
  version-2 Scalar `ActiveDatabaseRevision` and `DeployableRevision` facts
  exercise the closed production-private `validate_candidate_preflight` path
  called by normal apply. Its unit proof establishes expected-base,
  standard-context, then the exact
  `CandidateRevisionInvariant` wrapper before materialisation or physical
  planning; existing public version-1 and version-1-to-version-2 integrations
  retain expected-base, context, and no-write transaction coverage. Until the
  later atomic-standard-apply row, the legacy encoder rejects `Value(TypeId)`;
  the strict preflight prevents a version-2 Scalar from reaching it. The atomic
  row supplies the first installed-standard authority and proves atomic install
  followed by normal same-context apply, exact catalogue context and function
  semantic versions, all field, parameter, `ROWS`, and `SINGLE` value tuples, a
  non-golden `TypeId`, and the `ValueType` reference target pin, followed by
  post-write recovery and restart.
  It proves version-1 bytes, hashes, rows, and errors unchanged; migration-0008
  bytea declarations without defaults, all eight named length checks, all four
  named type-kind and tuple replacement checks, all eight named deferrable
  composite foreign keys, and unchanged object foreign keys; wrong, missing,
  and crossed pin or value rows in the stated tuple, pin, definition, canonical,
  then physical order; exhaustive recovery helper and fixture matches without a
  wildcard; the private-field `PhysicalCatalogue` boundary with only ordered
  `CreateObject` and `CreateField` accessors across crates; unsupported,
  transient, and `VOID` contracts in the stated physical-plan order; and one
  allowed contract with no physical DDL for a version-1-to-version-2 transition
  that has the same representation. The later atomic-apply and standard-lifecycle
  tracers use the installed-standard authority; neither creates a second
  standard, digest, or contract authority. The atomic-apply row alone proves
  the private encoder's two-caller reuse;
...
remains the recovery wrapper and retains its existing display. The preceding
transition-safety row puts expected-base comparison, standard-context guarding,
and strict persistence validation in one closed production-private
`validate_candidate_preflight(active, candidate)` path called by real apply.
Constructor-valid active and deployable version-2 Scalar values prove its exact
expected-base, context, then
...
order before materialisation or physical planning. Existing public integration
tests retain the expected-base and context no-write transaction coverage. The
first public same-context version-2 SQL tracer remains later, after decoder
support and the single standard installer authority exist.

Until the later atomic-standard-apply row, normal materialisation keeps its
current legacy encoder and rejects `Value(TypeId)`. The strict preflight above
therefore prevents a version-2 Scalar from reaching that encoder. The atomic
row then introduces the one shared PostgreSQL materialisation encoder for both
atomic installation and normal apply. It persists the candidate context and
does not use migration defaults for a version-2 candidate. `materialize`
retains the candidate's exact `CatalogueHashContext`, and
`verify_candidate_hashes` uses `catalogue_digest_with_context` with that
retained context rather than the version-1-only `catalogue_digest`. The encoder
writes
...
apply cannot persist version-1 defaults. Atomic standard apply and normal apply
use this one private, context-aware encoder. Atomic apply retains separate
orchestration and the compiler-proven invariant rule; its row owns the actual
two-caller reuse proof.
...
| `fix(postgres): reject transitional version-two scalar candidates` | `crates/orna-kernel-postgres/src/apply.rs`, `crates/orna-kernel-postgres/src/lib.rs` | Add `PostgresKernelError::CandidateRevisionInvariant(RevisionInvariantError)` with its exact source wrapper. The production-used private `validate_candidate_preflight`, called by normal apply, checks expected base, standard context, then `validate_persistable_catalogue` before materialisation, physical planning, or SQL. Its `apply.rs` unit proof uses constructor-valid active and deployable version-2 Scalar values to establish that exact order and wrapper. Existing public version-1 apply and version-1-to-version-2 integration tests retain no-write expected-base and context coverage. |
...
| `test(postgres): migrate final version-two value fixtures` | `docs/decisions/0016-catalogue-backed-standard-types.md`, `crates/orna-kernel-postgres/src/apply.rs`, `crates/orna-kernel-postgres/src/physical/verify.rs` | Before core closes construction, remove the last PostgreSQL unit fixtures that require a constructor-valid version-2 `Scalar`. The normal-apply preflight proof uses a valid pinned `Value(TypeId)`, retains exact expected-base and context precedence, and reaches its later physical rule without changing production. Physical verification compares complete version-1 `Scalar` and version-2 `Value(TypeId)` projections for the same verified contract and requires identical PostgreSQL facts. It adds no raw database fixture, scalar-to-identity map, contract mapper, or production change. |
| `feat(core): close version-2 scalar compatibility` | `crates/orna-core/src/canonical_hash.rs`, `crates/orna-core/src/revision.rs` | After compiler emission, raw recovery, client fixtures, and PostgreSQL unit fixtures migrate every current version-2 scalar to `Value(TypeId)`, both public canonical hash entry points reject every version-2 scalar in exact field, every function parameter, `ROWS`, and `SINGLE` slot order before bytes, and revision construction rejects it before existing coherence checks. Version-1 scalar behaviour remains byte- and error-identical. |
...
| `feat(postgres): apply standard upgrades` | `crates/orna-kernel-postgres/src/apply.rs`, `crates/orna-kernel-postgres/src/lib.rs`, `crates/orna-kernel-postgres/tests/apply.rs` | After compiler, recovery, storage, and execution consumers are ready, atomic special apply introduces the one shared context-aware encoder and proves actual two-caller reuse with normal apply. `materialize` retains the candidate context, candidate hashes use `catalogue_digest_with_context`, and the encoder writes exact catalogue version and pin, immutable semantic versions, value tuples, and value-reference pins; version 1 writes its explicit old tuples and null pins. `apply_standard_upgrade` accepts only `&orna_standard::StandardUpgrade`. Its trusted transaction path locks and recovers, checks expected base, scans database-wide identities, materialises, plans physically, then writes. Its live PostgreSQL tracer atomically installs, then normal same-context apply proves exact field, parameter, `ROWS`, and `SINGLE` rows, a non-golden `TypeId`, the `ValueType` target pin, recovery, and restart through the one installed-standard authority. Compiler construction already proves deployable core invariants, so no opaque-association or invariant gate exists. A replay returns `ExpectedBaseMismatch` before scanning or writes. The identity scan returns `ReservedStandardIdentity { identity }` for the first active-visible or inactive collision, including an inactive standard-library revision, in the stated order. Normal apply cannot transition standard context. |
...
| `test(server): prove the standard database lifecycle` | `crates/orna-server/tests/standard_database.rs`, `justfile` | The public opener drives a fresh database from bare version 1 through the one retained standard authority, proves that reopening the installed version-2 database and reconnecting preserve the exact active pair and accepted pinned catalogue and digest, and rejects a post-install standard-definition tamper without repair. The committed atomic-apply tracer remains the existing-version-one upgrade, replay, same-context Value-tuple and exact physical-storage proof; the raw recovery fixtures remain decoder-only and are not copied. |
...
| `test(client): recover and evaluate standard Boolean constants` | `crates/orna-kernel-postgres/Cargo.toml`, `crates/orna-kernel-postgres/tests/recovery.rs`, `Cargo.lock` | Through the public evaluator, a live PostgreSQL lifecycle applies a production-valid version-1 schema-only base, atomically installs the retained standard, applies a CLIENT Boolean constant, replays source-only changes without allocating a function revision, applies TRUE-to-FALSE semantic change with a new revision, recovers after restart, and rejects an exact post-install Boolean-definition tamper without repair. Every accepted version-2 function return and `NamedType` reference retains the exact selected pinned Boolean `ValueType` identity, every evaluation reports the exact active pair, function revision, and Boolean value, and every test session is shut down. |
