...
| 0092 | Function-backed CLI sessions use one thin client/session path for local, daemon, and remote endpoints; the no-command form targets the source-authored `std.cli.repl`, and `-d` is foreground daemon mode. | The endpoint transport, persistent action loop, native session frame wiring, remote TLS/auth, and production artifact trust remain implementation-gated. |
