...
| `build(postgres): embed lifecycle inputs` | `packaging/postgresql/embedded-build.toml`; `packaging/postgresql/embedded-postgresql-18.4.patch`; `packaging/postgresql/build-embedded.sh` | Add the renamed linked initialiser, direct fork-and-entry check/bootstrap/single-user seams, fixed bootstrap-collation assertion, executable-load guard, seccomp entry, and closed data-only support bundle. The builder accepts one explicit caller-owned output root and writes the verified Cargo link inputs there without relying on ignored `target/postgresql-embedded/current`. |
