...
| `chore(postgres): stage embedded patch series one` | `packaging/postgresql/embedded-postgresql-18.4/0001-linked-backend-entry-and-archive.patch`; `packaging/postgresql/embedded-postgresql-18.4/0002-embedded-runtime-capabilities-and-seccomp.patch`; `packaging/postgresql/embedded-postgresql-18.4/0003-fixed-postmaster-support-paths.patch` | Stage the independently reviewable linked backend entry and archive, embedded process capabilities and executable-load filter, and fixed postmaster support-path patches. Each patch applies after its exact predecessor with zero context and `--fuzz=0`; the monolithic patch remains the sole active builder input. |
| `chore(postgres): stage embedded patch series two` | `packaging/postgresql/embedded-postgresql-18.4/0004-executable-sql-and-loader-guard.patch`; `packaging/postgresql/embedded-postgresql-18.4/0005-initdb-direct-check-and-bootstrap.patch`; `packaging/postgresql/embedded-postgresql-18.4/0006-fixed-bootstrap-collation-and-no-plpgsql.patch` | Stage the executable-SQL and dynamic-loader guard, direct linked `--check` and `--boot` child runner with initialisation-child output suppression, and fixed UTC seven-collation bootstrap without PL/pgSQL. Each patch applies after its exact predecessor with zero context and `--fuzz=0`; the monolithic patch remains active. |
| `chore(postgres): stage embedded patch series three` | `packaging/postgresql/embedded-postgresql-18.4/0007-initdb-fixed-support-paths.patch`; `packaging/postgresql/embedded-postgresql-18.4/0008-initdb-single-user-entry-and-archive.patch` | Stage fixed initialiser support-root selection and the direct `--single` setup, typed initialiser ABI, namespace forwarding, and flattened initialiser archive. Both patches apply after their exact predecessors with zero context and `--fuzz=0`; the monolithic patch remains active. |
| `build(postgres): freeze embedded patch series` | `packaging/postgresql/embedded-build.toml`; `packaging/postgresql/build-embedded.sh`; `packaging/postgresql/embedded-postgresql-18.4.patch` | Delete the monolithic PostgreSQL patch and select the previously staged ordered zero-context series. Freeze, SHA-256-verify, dry-run, and apply each patch in order with `--fuzz=0`; bind every frozen patch byte and its order in the deterministic manifest; preserve the linked archives, symbols, support bundle, probe, licence, caller-owned output, and no-executable output. |
| `fix(postgres): remove argv0 service authority` | `packaging/postgresql/embedded-postgresql-18.4/0009-remove-argv0-locale-and-service-authority.patch`; `packaging/postgresql/embedded-build.toml`; `packaging/postgresql/build-embedded.sh` | Append one independently pinned patch that removes both linked calls to `set_pglocale_pgservice`, clears `PGSYSCONFDIR` and `PGLOCALEDIR` before each private entry, preserves the backend fixed locale and initialiser `locale=C`, and proves no linked entry derives locale or configuration authority from `argv[0]`. |
| `build(postgres): prove embedded initialisation` | `.github/workflows/postgresql-embedded.yml`; `packaging/postgresql/build-embedded.sh`; `packaging/postgresql/embedded-build.toml` | Build twice, run the linked initialiser and postmaster tracer, prove exact cluster and support facts, and trace no PostgreSQL executable, shared object, executable mapping, or download. |
