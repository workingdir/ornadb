...
| `feat(compiler): resolve unique Text selectors` | `crates/orna-compiler/src/relational.rs`; `crates/orna-compiler/src/resolver.rs`; `crates/orna-compiler/src/resolver/model.rs` | Resolve only the exact unique Text selector, retain its identities and evidence, and select the separate query model. |
